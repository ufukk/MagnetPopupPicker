...
-(void)selectFirstElement
{
    if(self.filteredOptions.count > 0)
        self.selectedPair = [[KeyValuePair alloc] initWithKeyValue:[[self.filteredOptions objectAtIndex:0] valueForKey:self.keyNames.key] value:[[self.filteredOptions objectAtIndex:0] valueForKey:self.keyNames.value]];
}
...
{
    self.filteredOptions = [self.optionList filteredArrayUsingPredicate:[NSPredicate predicateWithBlock:^BOOL(KeyValuePair *evaluatedObject, NSDictionary *bindings) {
...
-(void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component
{
    if(row < self.filteredOptions.count)
        self.selectedPair = [[KeyValuePair alloc] initWithKeyValue:[[self.filteredOptions objectAtIndex:row] valueForKey:self.keyNames.key] value:[[self.filteredOptions objectAtIndex:row] valueForKey:self.keyNames.value]];
}
